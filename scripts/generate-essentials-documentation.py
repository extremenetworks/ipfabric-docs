...
def main():
    for action in ACTIONS:
        ...
        result.append(HORIZONTAL_BORDER)
        # print "setting up results body to be length " + str(len(action['parameters']))
        ...
        for name, values in action['parameters'].items():
            if values.get('description'):
                ...
                try:
                    # print "inserting record at position " + str(values['position'])
                    position = values['position']
                    results_body[position] = '\n'.join(details)
                except KeyError:
                    print values['name'] + " appears to be missing a position value in " + action['name']
                    sys.exit(1)
                except IndexError:
                ...
                print "Parameter " + name + " does not have a description in " + action['name']
                sys.exit(1)
        ...
        try:
            ...
            print "uh-oh, throwing a TypeError when writing " + file_name
            print "Most something messed up with position parameters"
            # for key in action:
            #     print key, 'corresponds to', action[key]
